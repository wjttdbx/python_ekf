...
def create_ekf(noisy: bool, x0: np.ndarray) -> RelativeStateEKF:
    """创建 EKF 实例。
    ...
    """
    # 测量噪声标准差
    sigma_rho = (10.0 / 1000.0) if noisy else 0.0   # 10 m → km
    sigma_ang = (1e-4 * DEG2RAD) if noisy else 0.0  # 1e-4 deg → rad

    R_meas = np.diag([
        max(sigma_rho**2, 1e-30),
...
def run_simulation(noisy: bool = True):
    ...
    ekf = create_ekf(noisy=noisy, x0=x0_est)
