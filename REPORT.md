...
| 指标 | 值 |
|------|-----|
| 捕获时刻 | 10.38 h（约 2.04 个轨道周期） |
| 捕获距离 | 99.8 m |
| EKF 位置误差均值 | 6.88 m |
| EKF 位置误差最大值 | 35.49 m |
| 追踪星推力均值 / 峰值 | 193.1 / 2338.8 μm/s² |
| 逃逸星推力均值 / 峰值 | 96.5 / 1169.4 μm/s² |
...
有噪声与无噪声捕获时刻相差 0.04 h（约 2.4 min），说明 EKF 估计质量良好，测量噪声对控制性能影响极小。

### EKF 性能分析

- 位置估计误差均值 6.88 m，小于测量噪声 $\sigma_\rho = 10$ m，滤波有效
- 误差始终在 3σ 包络内，滤波器一致性（consistency）良好
- z 方向误差在 2h 后收敛至 ±2 m；x/y 方向因椭圆轨道动力学耦合较强，误差维持在 ±15 m 振荡
- 新息 δaz 在捕获前（距离 < 1 km）出现尖峰，为方位角奇异性导致，属正常物理现象
