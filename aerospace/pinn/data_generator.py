"""
3D SDRE-PINN 数据生成与预处理模块

输入特征 (10D)：从 6×6 A_SDC 提取 10 个独立非平凡元素
    [A[3,0], A[3,1], A[3,2], A[3,4], A[4,0], A[4,1], A[4,2], A[5,0], A[5,1], A[5,2]]
输出标签 (21D)：Log-Cholesky 参数（6 个对角取 log，15 个非对角直接输出）
"""
...
STATE_DIM = 6
CTRL_DIM = 3
FEAT_DIM = 10  # A_SDC 独立非平凡元素
L_DIM = 21     # tril(6×6)

EARTH_RADIUS_KM = 6371.0   # 地球半径，用于过滤地球内部点
CAPTURE_DIST_KM = 0.1      # 追逃距离阈值：100 m = 0.1 km

_TRIL_R, _TRIL_C = np.tril_indices(STATE_DIM)
_DIAG_MASK = (_TRIL_R == _TRIL_C)  # 6 个 True，15 个 False


def extract_asdc_features(a_sdc: np.ndarray) -> np.ndarray:
    """从 6×6 A_SDC 提取 10 个独立非平凡元素。

    A_SDC 上半部为 [0|I₃]，下半部结构中 A[4,3]=-A[3,4] 冗余。
    """
    return np.array([
        a_sdc[3, 0], a_sdc[3, 1], a_sdc[3, 2], a_sdc[3, 4],
        a_sdc[4, 0], a_sdc[4, 1], a_sdc[4, 2],
        a_sdc[5, 0], a_sdc[5, 1], a_sdc[5, 2],
    ], dtype=np.float64)


def _solve_are_balanced(A: np.ndarray, B: np.ndarray,
                        Q: np.ndarray, R_eff: np.ndarray) -> np.ndarray:
    """辛平衡 + 状态缩放两级预处理的 ARE 求解。"""
    ...
    d = np.array([1.0 / max(np.max(np.abs(A[i, :])), 1e-15) for i in range(A.shape[0])])
    T = np.diag(d)
    Ti = np.diag(1.0 / d)
...
def _p_to_log_cholesky_vec(p: np.ndarray) -> np.ndarray:
    """P → Log-Cholesky 21-vec（对角取 log，非对角直接输出）。"""
...
class PINNDataset(Dataset):
    def __init__(self, feat_norm: np.ndarray, l_norm: np.ndarray, a_sdc: np.ndarray,
                 p_true: np.ndarray | None = None, x_rel: np.ndarray | None = None):
...
def _is_spd(mat: np.ndarray, eps: float = 1e-10) -> bool:
    if not np.allclose(mat, mat.T, atol=1e-7):
        return False
    return bool(np.all(np.linalg.eigvalsh(mat) > eps))


def _above_earth(x: np.ndarray, r_c: float) -> bool:
    """检查 LVLH 坐标下的航天器是否在地球表面以上。

    LVLH 中 x[0] 为径向偏差，地心距 = r_c + x[0]（径向）加横向分量。
    """
    r = np.sqrt((r_c + x[0])**2 + x[1]**2 + x[2]**2)
    return r > EARTH_RADIUS_KM


def _lhs_positions_3d(n_samples: int, bounds: SamplingBounds, seed: int) -> np.ndarray:
    """3D LHS 采样绝对位置 [x, y, z]。"""
    sampler = qmc.LatinHypercube(d=3, seed=seed)
    u = sampler.random(n=n_samples)
    low = np.array([bounds.pos_min_km] * 3, dtype=float)
    high = np.array([bounds.pos_max_km] * 3, dtype=float)
    return qmc.scale(u, low, high)


def _lhs_orbital_params(n_samples: int, bounds: SamplingBounds, seed: int) -> np.ndarray:
...
def _run_rollouts_3d(
    ...
    """运行 3D 闭环轨迹回放，收集真实流形上的训练样本。

    终止条件：
      1. 追逃相对距离 < CAPTURE_DIST_KM（100 m）
      2. 仿真步数超过 10 个轨道周期对应的步数
      3. 航天器进入地球内部
    """
    ...
    for traj_i in range(n_rollouts):
        pos_p = rng.uniform(-500, 500, size=3)
        pos_e = rng.uniform(-500, 500, size=3)
        vel_p = rng.uniform(-0.05, 0.05, size=3)
        vel_e = rng.uniform(-0.05, 0.05, size=3)
        nu0 = rng.uniform(0, 2 * np.pi)

        a_c = rng.uniform(bounds.ac_min_km, bounds.ac_max_km)
        e_c = rng.uniform(bounds.ec_min, bounds.ec_max)
        dynamics = OrbitalDynamics(mu=3.986e5, a_c=a_c, e_c=e_c)

        # 初始点地球内部检查
        r_c0, _, _ = dynamics.get_orbital_params(nu0)
        X_p0 = np.concatenate([pos_p, vel_p])
        X_e0 = np.concatenate([pos_e, vel_e])
        if not _above_earth(X_p0, r_c0) or not _above_earth(X_e0, r_c0):
            fail_count += 1
            continue

        state = np.zeros(13)
        state[0:3], state[3:6] = pos_p, vel_p
        state[6:9], state[9:12] = pos_e, vel_e
        state[12] = nu0

        # 10 个轨道周期对应的最大步数
        max_steps = int(10 * dynamics.T_orbit / dt)

        for _ in range(max_steps):
            X_p, X_e = state[0:6], state[6:12]
            nu_val = state[12]
            x_rel = X_p - X_e

            # 捕获终止：追逃距离 < 100 m
            if np.linalg.norm(x_rel[:3]) < CAPTURE_DIST_KM:
                break

            r_c, nu_dot, nu_ddot = dynamics.get_orbital_params(nu_val)

            # 地球内部检查
            if not _above_earth(X_p, r_c) or not _above_earth(X_e, r_c):
                fail_count += 1
                break
            ...
            try:
                sol = solve_ivp(
                    lambda _t, y: dynamics.dynamics_13d(_t, y, u_p, u_e),
...
def generate_dataset(
    ...
    output_path: str = "data/sdre_pinn_dataset.npz",
    val_ratio: float = 0.2,
    seed: int = 42,
    ...
    """生成 3D SDRE-PINN 数据集（LHS 随机采样 + 闭环轨迹回放混合）。

    通过 LHS 采样位置和真近点角生成多样的 A_SDC 矩阵，
    然后从 A_SDC 中提取 10 个独立参数作为网络输入特征。
    ...
    pos_p_samples = _lhs_positions_3d(n_samples, bounds, seed=seed)
    pos_e_samples = _lhs_positions_3d(n_samples, bounds, seed=seed + 13)
    ...
    for i in range(n_samples):
        ...
        X_p = np.array([pos_p[0], pos_p[1], pos_p[2], 0.0, 0.0, 0.0])
        X_e = np.array([pos_e[0], pos_e[1], pos_e[2], 0.0, 0.0, 0.0])
        ...
        feat = extract_asdc_features(a_sdc)  # (10,)
    ...
    if n_rollouts > 0:
        ...
        rf, rp, ra, rx, rfail = _run_rollouts_3d(
...
def main() -> None:
    generate_dataset(
        n_samples=500_000,
        output_path="data/sdre_pinn_dataset.npz",
        val_ratio=0.2,
        seed=42,
    )
