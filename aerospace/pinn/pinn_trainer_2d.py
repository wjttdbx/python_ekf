...
输入 5D A_SDC 特征 → 残差主干 (3×ResBlock 256) → 10 个独立任务头
...
from aerospace.pinn.data_generator_2d import build_dataloaders, STATE_DIM, CTRL_DIM, L_DIM


# 10 维 L 向量中对角元素的索引: (0,0)→0, (1,1)→2, (2,2)→5, (3,3)→9
DIAG_INDICES = [0, 2, 5, 9]
OFFDIAG_INDICES = [1, 3, 4, 6, 7, 8]


def log_cholesky_to_cholesky_l(vec: torch.Tensor) -> torch.Tensor:
    """(B, 10) Log-Cholesky 向量 → (B, 4, 4) 下三角矩阵。

    对角位置 (idx 0,2,5,9) 通过 exp() 恢复正值，非对角直接填入。
    """
    bsz = vec.shape[0]
...
def reconstruct_spd_p(vec: torch.Tensor, delta: float = 1e-4) -> torch.Tensor:
    """由 10 维 Log-Cholesky 输出重构 SPD P (B, 4, 4)。"""
...
class TaskHead(nn.Module):
    """独立任务头: Linear(backbone_dim → 64) → Mish → Linear(64 → 1)"""
...
class SDREPINN2D(nn.Module):
    """残差主干 + 10 路独立任务头

    Backbone: Linear(in_dim → 256) + Mish + 3×ResBlock(256)
    Heads:    10 × TaskHead(256 → 64 → 1)
    """
    ...
    def __init__(
        ...
        n_resblocks: int = 3,
        head_hidden: int = 64,
        activation: str = "mish",  # kept for checkpoint compatibility
        dropout: float = 0.0,
    ):
        super().__init__()
        self.in_dim = in_dim
    ...
    def forward(self, feat: torch.Tensor) -> torch.Tensor:
        h = self.proj(feat)
        h = self.resblocks(h)  # (B, backbone_dim)
        outputs = [head(h) for head in self.heads]  # list of (B, 1)
        return torch.cat(outputs, dim=1)  # (B, 10)


@dataclass
class TrainConfig2D:
    dataset_path: str = "data/sdre_pinn_2d_dataset.npz"
    output_dir: str = "checkpoints/sdre_pinn_2d"
    epochs: int = 600
    batch_size: int = 8192
    num_workers: int = 8
...
def _maybe_compile_model(model: nn.Module, config: TrainConfig2D) -> nn.Module:
...
def compute_losses(l_pred_norm, l_true_norm, p_pred, p_true, x_rel,
    ...
    # L_u: 控制量绝对均方误差 (MSE) 代替相对误差
    K_batch = R_inv_Bp_T.unsqueeze(0).expand(bsz, -1, -1)  # (B, CTRL, STATE)
...
def train_pinn(config: TrainConfig2D) -> dict:
...
def _plot_loss_curves(history: dict, output_dir: Path) -> None:
    ...
    fig.suptitle("2D PINN Training Loss Curves", fontsize=14)
    plt.tight_layout()
