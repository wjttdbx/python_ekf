...
OUTPUT_DIR = Path("results/validate_2d")
...
def _make_test_states(n=500, seed=9999):
    ...
    pos_p = qmc.scale(sampler_p.random(n=n), [-1000]*2, [1000]*2)
    sampler_e = qmc.LatinHypercube(d=2, seed=seed+3)
    pos_e = qmc.scale(sampler_e.random(n=n), [-1000]*2, [1000]*2)
    sampler_nu = qmc.LatinHypercube(d=1, seed=seed+7)
    nu = 2 * np.pi * sampler_nu.random(n=n).reshape(-1)

    b_p = np.zeros((STATE_DIM, CTRL_DIM)); b_p[2:, :] = np.eye(CTRL_DIM)
    q = np.eye(STATE_DIM)
    ...
    results = []
    for i in range(n):
        X_p = np.array([pos_p[i,0], pos_p[i,1], 0, 0])
        X_e = np.array([pos_e[i,0], pos_e[i,1], 0, 0])
...
def exp2_are_residual(p_batch, test_data, q, r, gamma):
    ...
    for i, (X_p, X_e, nu, a_sdc, p_true) in enumerate(test_data):
        ...
        norm_p = np.linalg.norm(P @ a_sdc, 'fro') + np.linalg.norm(P @ G @ P, 'fro') + np.linalg.norm(q, 'fro')
...
def exp3_elementwise(p_batch, test_data):
    ...
    print("  " + "-"*62)
    labels = [
        ((0,0),"P[0,0]"), ((0,1),"P[0,1]"), ((0,2),"P[0,2]"), ((0,3),"P[0,3]"),
...
def exp4_control_consistency(ctrl_neural, test_data, q, r, gamma):
    ctrl_sdre = SDREGameController2D(q, r, gamma)
    b_p = np.zeros((STATE_DIM, CTRL_DIM)); b_p[2:, :] = np.eye(CTRL_DIM)
    r_eff = r / (1.0 - gamma**(-2))
    ...
    ue_errs = np.array(ue_errs)
    up_rel = np.array(up_rel_errs)
    ue_rel = np.array(ue_rel_errs)
...
def exp5_closed_loop(checkpoint, dynamics, q, r, gamma):
    scenarios = [
        {"name": "A: 追方500km后方", "x_p0": [500.0, 500.0, 0.01, 0.01], "x_e0": [0,0,0,0], "nu0": 0.0},
        {"name": "B: 追方100km近距",  "x_p0": [100.0,  50.0, 0.005, 0.005], "x_e0": [0,0,0,0], "nu0": 0.5},
        {"name": "C: 逃方偏心轨道",   "x_p0": [300.0, 200.0, 0.0, 0.0], "x_e0": [-100,50,0.001,-0.001], "nu0": 1.0},
    ]
    ...
    for sc in scenarios:
        ...
        sdre_sim = SDRESimulation2D(dynamics, sdre_ctrl, x_p0, x_e0, nu0=sc["nu0"], dt=20.0, log_interval=9999)
        sdre_res = sdre_sim.run()

        neural_ctrl = NeuralSDREController2D(checkpoint_path=checkpoint)
        neural_sim = SDRESimulation2D(dynamics, neural_ctrl, x_p0, x_e0, nu0=sc["nu0"], dt=20.0, log_interval=9999)
        neural_res = neural_sim.run()

        sdre_dist = np.linalg.norm(sdre_res.states[0:2, -1] - sdre_res.states[4:6, -1])
        pinn_dist = np.linalg.norm(neural_res.states[0:2, -1] - neural_res.states[4:6, -1])
        ...
        print(f"  {sc['name']:>22s}  {sdre_dist:>14.3f}  {pinn_dist:>14.3f}  {dist_err:>12.2f}  "
              f"{sdre_dv:>10.3f}  {pinn_dv:>10.3f}  {dv_err:>10.2f}  {fb_str:>8s}")
...
def _plot_trajectory_comparison(all_results, output_dir):
    ...
    for idx, (ax, res) in enumerate(zip(axes, all_results)):
        sdre = res["sdre_res"]
        pinn = res["pinn_res"]
        ax.plot(sdre.states[0] - sdre.states[4], sdre.states[1] - sdre.states[5],
                label="SDRE (truth)", linewidth=2, alpha=0.8)
        ax.plot(pinn.states[0] - pinn.states[4], pinn.states[1] - pinn.states[5],
...
def _plot_distance_comparison(all_results, output_dir):
    ...
    for idx, (ax, res) in enumerate(zip(axes, all_results)):
        sdre = res["sdre_res"]
        pinn = res["pinn_res"]
        sdre_d = np.sqrt((sdre.states[0]-sdre.states[4])**2 + (sdre.states[1]-sdre.states[5])**2)
        pinn_d = np.sqrt((pinn.states[0]-pinn.states[4])**2 + (pinn.states[1]-pinn.states[5])**2)
    ...
    print(
        "  Distance plot saved: "
        f"{output_dir / '追逃相对距离对比图.png'}"
    )

def _plot_residual_hist(residuals, output_dir):
    fig, ax = plt.subplots(figsize=(7, 4))
    ax.hist(residuals, bins=50, edgecolor="black", alpha=0.7)
    ax.axvline(np.median(residuals), color="red", linestyle="--", label=f"median={np.median(residuals):.2e}")
    ax.axvline(np.percentile(residuals, 99), color="orange", linestyle="--", label=f"P99={np.percentile(residuals,99):.2e}")
    ax.set_xlabel("ARE Relative Residual")
    ax.set_ylabel("Count")
    ax.set_title("$P_{pred}$ ARE Residual Distribution")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    fig.savefig(output_dir / "ARE相对残差直方图.png", dpi=150)
    print(
        "  ARE residual plot saved: "
        f"{output_dir / 'ARE相对残差直方图.png'}"
    )
...
def main():
    ...
    print("\n" + "#"*60)
    print("#  2D PINN 模型正确性验证实验")
    print("#"*60)

    ctrl = NeuralSDREController2D(checkpoint_path=CHECKPOINT)
