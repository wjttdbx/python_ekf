...
def _generate_eval_states(
    ...
    """生成评测用两星绝对位置和真近点角。"""
    low = np.array([pos_min_km] * 3, dtype=float)
    high = np.array([pos_max_km] * 3, dtype=float)

    sampler_p = qmc.LatinHypercube(d=3, seed=seed)
    pos_p = qmc.scale(sampler_p.random(n=n_samples), low, high)

    sampler_e = qmc.LatinHypercube(d=3, seed=seed + 7)
...
def run_batch_inference_test(
    ...
    ctrl = NeuralSDREController(checkpoint_path=checkpoint_path)
    ...
    pos_p_batch, pos_e_batch, nu_batch = _generate_eval_states(
    ...
    print("3D Batch Inference Test Result:")
