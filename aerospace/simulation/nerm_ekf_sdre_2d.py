"""
2D NERM + EKF + SDRE 闭环仿真引擎

追踪星通过 EKF 估计相对状态，再由 2D SDRE 控制器计算面内推力。
"""
...
@dataclass
class EKFSDRESimResult2D:
    """2D EKF+SDRE 仿真结果数据容器。

    Attributes
    ----------
    t              : (N,)      时间序列 (s)
    states         : (9, N)    真实状态历史 [X_p, X_e, nu]
    x_est_history  : (4, N)    EKF 估计相对状态历史
    u_p_history    : (2, N)    追踪星推力历史
    u_e_history    : (2, N)    逃逸星推力历史
    dist_history   : (N,)      相对距离历史 (km)
    ekf_err_history: (4, N)    EKF 各分量估计误差历史 (km, km/s)
    innov_history  : (R_dim, N) EKF 新息历史
    P_diag_history : (4, N)    EKF 协方差对角元素历史
...
class EKFSDRESimulation2D:
    ...
    X_p0       : (4,) ndarray  追踪星初始状态
    X_e0       : (4,) ndarray  逃逸星初始状态
    ...
    def __init__(self, dynamics: OrbitalDynamics2D, controller: SDREGameController2D,
                 ekf: RelativeStateEKF2D,
        ...
        self._B_ctrl = np.zeros((4, 2))
        self._B_ctrl[2:, :] = np.eye(2)

    def run(self, t_end: float | None = None) -> EKFSDRESimResult2D:
        """执行闭环仿真。"""
        if t_end is None:
        ...
        t_hist        = np.zeros(N + 1)
        state_hist    = np.zeros((9, N + 1))
        x_est_hist    = np.zeros((4, N + 1))
        u_p_hist      = np.zeros((2, N + 1))
        u_e_hist      = np.zeros((2, N + 1))
        dist_hist     = np.zeros(N + 1)
        ekf_err_hist  = np.zeros((4, N + 1))
        innov_dim     = self.ekf.R.shape[0]
        innov_hist    = np.zeros((innov_dim, N + 1))
        P_diag_hist   = np.zeros((4, N + 1))
        ...
        def _record(k: int, st: np.ndarray, x_est: np.ndarray,
            ...
            u_p_hist[:, k]    = up
            u_e_hist[:, k]    = ue
            dist_hist[k]      = np.linalg.norm(st[0:2] - st[4:6])
            x_true_rel        = st[0:4] - st[4:8]
        ...
        for k in range(N):
            ...
            # ── 真实状态传播 ─────────────────────────────────────────────────
            sol = solve_ivp(
                self.dynamics.dynamics_9d, [t, t + self.dt], state,
            ...
            if self.rng is not None:
                ...
                _angle_only = (self.ekf.R.shape[0] == 1)
                z_true = RelativeStateEKF2D.measure(state[0:4], state[4:8], angle_only=_angle_only)
                ...
                self.ekf.x = state[0:4] - state[4:8]
