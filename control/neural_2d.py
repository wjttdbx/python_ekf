"""
2D 神经代理 SDRE 控制器与闭环性能对比

用 PINN 近似 4×4 ARE 在线求解，含 OOD 安全回退。
输入特征 (5D)：从 A_SDC 提取 [a20, a21, a23, a30, a31]。
...
from aerospace.pinn.pinn_trainer_2d import SDREPINN2D, reconstruct_spd_p
from aerospace.pinn.data_generator_2d import STATE_DIM, CTRL_DIM, extract_asdc_features
...
class NeuralSDREController2D:
    """2D PINN 代理控制器，接口与 SDREGameController2D 兼容。"""
    ...
    def set_environment(self, r_c: float, nu_dot: float, nu_ddot: float):
        """保留接口兼容性。"""
        pass

    def set_positions(self, X_p: np.ndarray, X_e: np.ndarray):
        """保留接口兼容性。"""
        pass

    def _build_feature(self, x_rel, a_sdc):
        """直接从 A_SDC 提取 5 个独立非平凡元素。"""
        return extract_asdc_features(a_sdc)

    def _is_ood(self, feat_raw):
        z = np.abs((feat_raw - self.feat_mean) / self.feat_std)
        return bool(np.any(z > self.ood_zscore_threshold))

    def _solve_are_fallback(self, a_sdc):
    ...
    def compute_control(self, A_SDC, x_rel, t=None, solve_are=True):
        self.total_calls += 1
    ...
    def predict_p_batch(self, feat_raw_batch, batch_size=1024):
        """批量预测 P 矩阵。feat_raw_batch 应为 (N, 5) A_SDC 特征。"""
...
def _relative_distance_km(states: np.ndarray) -> float:
    x_rel = states[0:2, -1] - states[4:6, -1]
...
def run_closed_loop_benchmark(
    ...
    dynamics = OrbitalDynamics2D(mu=3.986e5, a_c=24567, e_c=0.2346)

    q = np.eye(STATE_DIM)
    r = np.eye(CTRL_DIM) * 1e13
    gamma = np.sqrt(2)

    x_p0 = np.array([500.0, 500.0, 0.01, 0.01], dtype=float)
    x_e0 = np.zeros(STATE_DIM, dtype=float)

    baseline_ctrl = SDREGameController2D(q, r, gamma)
    baseline_sim = SDRESimulation2D(dynamics, baseline_ctrl, x_p0, x_e0, nu0=0.0, dt=dt)
    t0 = time.perf_counter()
    baseline_res = baseline_sim.run()
    baseline_time = time.perf_counter() - t0

    neural_ctrl = NeuralSDREController2D(checkpoint_path=checkpoint_path)
    neural_sim = SDRESimulation2D(dynamics, neural_ctrl, x_p0, x_e0, nu0=0.0, dt=dt)
    t1 = time.perf_counter()
    neural_res = neural_sim.run()
    neural_time = time.perf_counter() - t1

    steps = max(1, baseline_res.t.size)
    return SimBenchmarkResult2D(
...
def main():
    result = run_closed_loop_benchmark(
        checkpoint_path="checkpoints/sdre_pinn_2d/best_model.pt"
    )
    print("2D 闭环对比结果:", result)
