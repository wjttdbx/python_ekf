...
\begin{enumerate}
    ...
    \item \textbf{Characterization of a counterintuitive sensor noise effect.} We report and dissect the empirically observed finding that larger sensor noise accelerates approach (by a factor of 2.3 across $\sigma_\theta \in [0.001^\circ, 0.1^\circ]$) while peak thrust and total $\Delta V$ remain essentially constant. A naive linear-Gaussian analysis predicts no such effect and, in fact, the wrong sign for $\|K\|_F$; the actual mechanism is a composition of nonlinear filter--controller coupling through $A_{\text{SDC}}(\hat{\mathbf{x}})$, $\sigma_\theta$-scaled initial-covariance transients, and first-passage bias at inflated terminal velocities---a direct manifestation of the failure of LQG separation in the common SDC-matrix architecture (§5.2).
...
Table~\ref{tab:expB} reports the mean $\pm$ sample standard deviation across 5 independent seeds per noise level. We note that the within-level sample size ($n = 5$) is intended to characterize seed-to-seed variability under fixed initial conditions and controller parameters; the reported standard deviations should be interpreted as indicators of run-to-run reproducibility rather than as precise confidence intervals for population parameters. The uniformly small dispersion (relative std $\lesssim 1\%$ on all columns except terminal $v_{\text{rel}}$) reflects the deterministic sensitivity of the closed-loop response to noise magnitude once averaged over the ${\sim}10^4$-step approach horizon. With this caveat, three striking findings emerge:

\textbf{1. Sensor noise accelerates approach.} Increasing sensor noise by a factor of 100 (from $0.001^\circ$ to $0.1^\circ$) reduces approach time by a factor of 2.3 (86{,}994~s $\to$ 38{,}220~s), significant at $p < 0.01$ (Spearman's $\rho = -0.89$ over the 30 trials). The mechanism deserves careful examination because a naive linear-Gaussian analysis does not predict it and, in fact, points in the wrong direction. At fixed control weights $(Q_{\text{ctrl}}, R_{\text{ctrl}})$, the SDRE feedback law $\mathbf{u} = -R_{\text{ctrl}}^{-1} B^T P \hat{\mathbf{x}}$ is linear in the estimate. Under the standard certainty-equivalence assumption with unbiased Gaussian estimation error $\hat{\mathbf{x}} = \mathbf{x} + \mathbf{e}$, $\mathbf{e} \sim \mathcal{N}(\mathbf{0}, \hat{P})$, we have $\mathbb{E}[\mathbf{u}] = -R_{\text{ctrl}}^{-1} B^T P \mathbf{x}$ (\emph{unchanged} by $\sigma_\theta$) and only the second moment grows: $\mathbb{E}[\|\mathbf{u}\|^2] = \|R_{\text{ctrl}}^{-1} B^T P \mathbf{x}\|^2 + \text{tr}(R_{\text{ctrl}}^{-1} B^T P \hat{P} P B R_{\text{ctrl}}^{-1})$. Under the LQG separation principle applied to the SDC-linearized model, this variance term enters the expected cost as an additive control-effort penalty $\text{tr}(K^T R_{\text{ctrl}} K \hat{P})$ per unit time---it does \emph{not} rescale the effective $Q$-to-$R$ balance and does \emph{not} make the deterministic feedback more aggressive. If the classical separation principle held, we would expect a modest RMS growth in $\|\mathbf{u}\|$ but no material change in capture time.

Three empirical observations rule out the pure separation-principle picture and identify what actually drives the acceleration:

\textbf{(i) Total $\Delta V$ is invariant across the $\sigma_\theta$ sweep.} Total $\Delta V$ changes from 10.27~km/s to 10.19~km/s---a ${<}1\%$ change over a $100\times$ noise range. The 2.3$\times$ growth in \emph{mean} thrust reflects the same $\Delta V$ budget compressed into a shorter time interval ($\text{mean thrust ratio}=2.33\times$, essentially matching $\text{capture time ratio}=2.34\times$), not additional control effort. This decisively contradicts a "more aggressive control" reading of the effect. Peak thrust stays essentially fixed at $\|R_{\text{ctrl}}^{-1} B^T P\| \cdot \rho_0$, consistent with the ARE bound.

\textbf{(ii) The Kalman gain norm $\|K\|_F$ increases, not decreases, with $\sigma_\theta$.} Direct evaluation at mid-approach yields $\|K\|_F = 1.44 \times 10^{-3}$ at $\sigma_\theta = 0.001^\circ$ and $2.50 \times 10^{-3}$ at $0.1^\circ$ (a $1.7\times$ increase), even though $R_{\text{EKF}} \propto \sigma_\theta^2$ nominally attenuates the gain in a linear time-invariant filter. The paradoxical growth arises because the SDC matrix is evaluated at the estimate: $A_{\text{SDC}}(\hat{\mathbf{x}}_{k|k})$ depends on $\mathbf{e}$, which perturbs the ARE solution $P$ and hence $K$ in ways that the LTI Riccati analysis cannot predict. This is the \emph{nonlinear coupling between filter error and control gain that breaks the LQG separation principle}---the same coupling the paper's common-matrix architecture is built to acknowledge (§3.1).

\textbf{(iii) The initial covariance is $\sigma_\theta$-scaled.} By construction (§3.3.1), $P_0 = \text{diag}\big((\rho_0 \sigma_\theta)^2 \mathbf{I}_3, \sigma_\theta^2 \mathbf{I}_3\big)$, so the first EKF innovation and the first control update are proportionally larger for larger $\sigma_\theta$. This produces a stronger initial transient that pumps the chaser onto a fast closing trajectory before the filter converges. Combined with a terminal residual velocity that grows ${\sim}20\times$ with $\sigma_\theta$ (Table~\ref{tab:expB}, column $v_{\text{rel}}$), the chaser transits the $\rho < 100$~m capture threshold at speed---the first-passage time is bounded above by the mean arrival time, and a fast transit with a stochastic terminal distribution can achieve first crossing earlier than a slow soft-arrival trajectory.

Putting these together: what appears from Table~\ref{tab:expB} as "noise accelerates approach" is not a controller amplification effect at all. It is the composition of a nonlinear filter-controller coupling ($A_{\text{SDC}}$ evaluated at $\hat{\mathbf{x}}$), a $\sigma_\theta$-scaled initial-transient effect (through $P_0$), and a stochastic first-passage effect (through terminal $v_{\text{rel}}$). The classical linear-Gaussian intuition---"larger innovations produce larger corrections"---captures none of these mechanisms and would predict the wrong sign for $\|K\|$. We emphasize that this analysis is empirical, based on the numerical decomposition in Table~\ref{tab:expB} augmented by a two-point $\|K\|$ probe at $\sigma_\theta \in \{0.001^\circ, 0.1^\circ\}$; a fully analytical treatment of the nonlinear separation-principle failure remains open.
...
The inverse relationship between sensor noise and approach time (Table~\ref{tab:expB}) is an empirically observed phenomenon with potential operational relevance. Across the tested range ($\sigma_\theta \in [0.001^\circ, 0.1^\circ]$), a 100$\times$ relaxation in sensor precision reduces approach time by a factor of 2.3 while maintaining 100\% success rate and constant peak thrust. As dissected in §5.2, the driver is not "more aggressive control" in a certainty-equivalent sense---total $\Delta V$ is essentially invariant across the sweep---but a composition of nonlinear filter--controller coupling (through $A_{\text{SDC}}(\hat{\mathbf{x}})$), $\sigma_\theta$-scaled initial-transient effects (through $P_0 \propto \sigma_\theta^2$), and stochastic first-passage of the capture threshold at inflated terminal velocities. The phenomenon is therefore inseparable from the common SDC-matrix architecture itself: use a different filter-controller pair in which the filter estimate does \emph{not} enter the controller linearization point, and the effect would attenuate.
...
\begin{enumerate}
    ...
    \item \textbf{Sensor noise exhibits a counterintuitive acceleration effect that reflects nonlinear filter--controller coupling.} Increasing $\sigma_\theta$ from $0.001^\circ$ to $0.1^\circ$ (100$\times$) reduces approach time by a factor of 2.3 (Spearman's $\rho = -0.89$, $p < 0.01$) while peak thrust and total $\Delta V$ remain essentially constant (${<}1\%$ variation). A naive linear-Gaussian analysis predicts no acceleration and, in fact, the wrong sign for $\|K\|_F$: direct evaluation shows $\|K\|_F$ \emph{increases} $1.7\times$ across the sweep, not decreases as classical Riccati scaling would suggest, because the SDC matrix is evaluated at the noisy estimate $\hat{\mathbf{x}}$ and perturbs $P$ through the ARE. The actual driver of the acceleration is a composition of (a) $A_{\text{SDC}}(\hat{\mathbf{x}})$ coupling that breaks LQG separation, (b) $\sigma_\theta$-scaled initial covariance $P_0$ producing a strong first-innovation transient, and (c) first-passage bias at inflated terminal residual velocities. The phenomenon is therefore not a generic property of state feedback under measurement noise but a specific signature of the common SDC-matrix architecture; a dual-mode sensing strategy is suggested but further study is required before operational recommendations can be made.
\end{enumerate}
