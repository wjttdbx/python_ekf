...
\textbf{1. Sensor noise accelerates approach.} Increasing sensor noise by a factor of 100 (from $0.001^\circ$ to $0.1^\circ$) reduces approach time by a factor of 2.3 (87,000~s $\to$ 38,470~s), significant at $p < 0.01$ (Spearman's $\rho = -0.89$ over the 30 trials). The mechanism operates through two coupled pathways. \textit{Pathway A---innovation amplification:} larger $\sigma_\theta$ produces larger measurement innovations $\mathbf{y} - \hat{\mathbf{y}}$, which drive larger state corrections $K \cdot (\mathbf{y} - \hat{\mathbf{y}})$ through the Kalman update, even if the gain $K$ remains constant. \textit{Pathway B---gain adaptation:} the EKF's measurement noise covariance $R_{\text{EKF}}$ is matched to the true sensor noise ($R_{\text{EKF}} = \text{diag}(\sigma_\theta^2, \sigma_\theta^2)$); increasing $\sigma_\theta$ increases $R_{\text{EKF}}$, which \emph{decreases} the steady-state Kalman gain magnitude $\|K\|_F$. The observed acceleration indicates that Pathway~A (innovation amplification) dominates Pathway~B (gain reduction)---the larger innovations more than compensate for the attenuated gains. The net effect is confirmed by the monotonic increase in mean thrust (0.118 $\to$ 0.267~m/s$^2$, a factor of 2.3), while the average Kalman gain norm $\|K\|_F$ decreases by approximately 62\% over the same range. We emphasize that this effect is a consequence of the linear-state-feedback structure $\mathbf{u} = -R_{\text{ctrl}}^{-1}B^T P \hat{\mathbf{x}}$ operating on noise-perturbed estimates; it is not specific to the SDRE controller and would manifest with any state-feedback law that scales thrust with estimated state magnitude.
...
\textbf{Engineering implication:} For time-critical approach missions, sensor accuracy requirements are less stringent than conventionally assumed. An order-of-magnitude relaxation in sensor precision (from $0.001^\circ$ to $0.01^\circ$--$0.02^\circ$) reduces approach time by ${\sim}50\%$ with negligible increase in total $\Delta V$ and terminal velocities remaining within the sub-2~m/s soft-rendezvous regime.
