...
本课题的价值在于将 SDC 参数化从单一控制工具提升为估计与控制共享的动力学接口。若相对动力学满足 $f(x)=A_{\mathrm{SDC}}(x)x$，则在 EKF 当前后验估计 $\hat{x}_{k|k}$ 处构造的 $A_{\mathrm{SDC}}(\hat{x}_{k|k})$，既可作为 EKF 预测矩阵 $F_k=I+A_{\mathrm{SDC}}\Delta t$ 的来源，也可作为 SDRE 控制器求解 ARE 的系统矩阵。这不是简单的代码复用，而是由 SDC 框架的线性化点一致性决定的结构性设计。它可以减少模型失配、降低重复计算，并使闭环系统的估计和控制环节在同一非线性动力学表示下工作。
...
该问题包含三层含义。第一，在理论层面，需要证明 SDC 参数化的代数重构性质 $A(x)x\equiv f(x)$ 可以为估计器和控制器提供同一个局部动力学表示。传统 EKF 预测关注状态传播，SDRE 关注控制器设计，二者看似目标不同，但都需要在当前状态点构造一个可计算的局部系统矩阵。若二者使用不同矩阵，闭环内会产生模型不一致；若二者共享同一个 $A_{\mathrm{SDC}}$，则估计与控制在同一动力学假设下耦合。

第二，在方法层面，需要建立适用于三维椭圆轨道相对运动的 NERM/SDC 模型、仅测角 EKF 量测更新和 SDRE 控制律。该系统的状态维度必须保持一致：真值传播采用 13 维状态 $[X_p(6),X_e(6),\nu(1)]$，滤波与控制采用六维相对状态 $x_{\mathrm{rel}}=X_p-X_e$。只有严格保持状态定义、坐标轴和单位一致，才能避免仿真结果被符号或维度错误污染。
...
$$
x_{\mathrm{rel}}=X_p-X_e=[\Delta r^T,\Delta v^T]^T\in\mathbb{R}^6.
$$
...
$$
\dot{x}_{\mathrm{rel}}=f(x_{\mathrm{rel}},\nu,u_p,u_e).
$$
...
$$
\dot{x}_{\mathrm{rel}}=A_{\mathrm{SDC}}(x_{\mathrm{rel}},\nu)x_{\mathrm{rel}}+B(u_p-u_e),
$$
...
本课题的核心设计是：在每个离散时刻 $t_k$，根据 EKF 后验估计 $\hat{x}_{k|k}$ 和当前真近点角构造一次 $A_{\mathrm{SDC}}(t_k)$，然后同时用于两个环节：

$$
F_k=I+A_{\mathrm{SDC}}(t_k)\Delta t
$$

用于 EKF 状态和协方差预测；

$$
A_{\mathrm{SDC}}^TP+PA_{\mathrm{SDC}}-PBR^{-1}B^TP+Q=0
$$
...
其中 $v\sim\mathcal{N}(0,R_{\mathrm{EKF}})$，$R_{\mathrm{EKF}}=\operatorname{diag}(\sigma_\theta^2,\sigma_\theta^2)$。基准角度噪声取 $\sigma_\theta=0.008^\circ$。
...
$$
P_{k+1|k}=F_kP_{k|k}F_k^T+Q_{\mathrm{EKF}},\quad F_k=I+A_{\mathrm{SDC}}(t_k)\Delta t.
$$

更新步在预测状态处对量测函数 $h(x)$ 线性化，构造雅可比矩阵 $H_k=\partial h/\partial x$，并计算

$$
K_k=P_{k+1|k}H_k^T(H_kP_{k+1|k}H_k^T+R_{\mathrm{EKF}})^{-1},
$$
...
$$
J=\frac{1}{2}\int_0^T(x^TQ_{\mathrm{ctrl}}x+u_p^TR_{\mathrm{ctrl}}u_p-\gamma^2u_e^TR_{\mathrm{ctrl}}u_e)\,dt.
$$

在名义接近任务中，目标器可设为不机动，或将逃逸项作为博弈控制扩展。基于 $A_{\mathrm{SDC}}$，SDRE 控制器逐点求解 ARE，得到 $P(x)$ 后生成反馈控制律

$$
u_p=-R_{\mathrm{ctrl}}^{-1}B^TP\hat{x}_{k|k}.
$$

控制权重采用 $Q_{\mathrm{ctrl}}=I_6$，$R_{\mathrm{ctrl}}=10^{13}I_3$，以将推力限制在低加速度量级。由于 $Q_{\mathrm{ctrl}}$ 与 $B R_{\mathrm{ctrl}}^{-1}B^T$ 的尺度差异很大，直接求解 ARE 容易病态。为提高数值稳定性，控制器采用尺度平衡策略：令

$$
\alpha=\sqrt{\frac{\|Q_{\mathrm{ctrl}}\|}{\|B R_{\mathrm{ctrl}}^{-1}B^T\|}},
$$

对 $Q$ 和 $R$ 做一致缩放，求得平衡后的 $\bar{P}$，再恢复 $P=\alpha\bar{P}$。当平衡仍不能保证求解稳定时，可进一步对 $A_{\mathrm{SDC}}$ 进行行列尺度调整。该处理不是改变控制目标，而是改善数值条件。
...
总体流程如图2所示。每个控制周期包含七个步骤：读取当前 EKF 后验估计；根据 $\hat{x}_{k|k}$ 和 $\nu_k$ 构造 $A_{\mathrm{SDC}}$；用同一矩阵求解 SDRE ARE；生成追踪器控制输入；用 13 维 NERM 真值动力学传播真实状态；生成带噪声的仅测角量测；用同一 $A_{\mathrm{SDC}}$ 完成 EKF 预测并结合量测更新。该流程保证估计器和控制器共享同一动力学表达。
...
基准场景采用 MEO 椭圆参考轨道：$\mu=3.986\times10^5\ \mathrm{km^3/s^2}$，$a_c=15000$ km，$e_c=0.5$。由 Kepler 周期公式计算，轨道周期约为 $1.83\times10^4$ s，即约 5.08 h。追踪器初始状态为 $[500,500,500,0.01,0.01,0.01]$，目标器初始位于 LVLH 原点且速度为零，初始相对距离约为 866 km。控制周期为 $\Delta t=10$ s，捕获阈值为 100 m。

EKF 参数设置为：角度噪声基准 $\sigma_\theta=0.008^\circ$，过程噪声协方差为

$$
Q_{\mathrm{EKF}}=\operatorname{diag}(5\times10^{-4}I_3,5\times10^{-8}I_3).
$$

初始协方差按角度不确定性与初始距离的乘积设置，位置初始标准差约为 $\rho_0\sigma_\theta$，速度初始标准差取 $1.0\sigma_\theta$。SDRE 参数为 $Q_{\mathrm{ctrl}}=I_6$，$R_{\mathrm{ctrl}}=10^{13}I_3$，博弈参数 $\gamma=\sqrt{2}$。
...
| 控制 | $Q_{\mathrm{ctrl}}$ | $I_6$ | 状态权重 |
| 控制 | $R_{\mathrm{ctrl}}$ | $10^{13}I_3$ | 推力惩罚 |
...
实验 D 为轨道高度敏感性分析。LEO、MEO 和 GEO 三类轨道采用相同控制参数与传感器参数。MEO 基准场景表现稳健；LEO 中全状态信息 SDRE 和仅测角 EKF+SDRE 均失败，原因是快动力学导致 Coriolis 加速度较大，固定 $R_{\mathrm{ctrl}}=10^{13}I_3$ 下末端控制不足；GEO 中多数案例能捕获，但仅测角 EKF 可能因慢动力学、协方差冻结和角度绕卷产生异常案例。这说明统一 SDC 框架不是无需调参的全轨道通用方案，轨道高度和控制权重需要联合设计。
...
从计算可行性看，统一 SDC 矩阵方案每步只需构造一次 $A_{\mathrm{SDC}}$，该矩阵同时进入 EKF 与 SDRE，减少了重复线性化。ARE 求解仍是主要计算负担，但在 Python 仿真中已具备秒级运行能力，后续可通过稀疏 ARE 更新、C++ 实现或神经代理进一步降低星载部署成本。
...
主要风险有四项。第一，SDC 分解非唯一，不同参数化会导致不同 ARE 解和闭环轨迹。本报告采用代码中的标准分式因子化，后续需要开展参数化敏感性研究。第二，仅测角可观测性依赖几何激励，远距离或不利初始构型会导致 EKF 发散。第三，固定 $Q_{\mathrm{ctrl}},R_{\mathrm{ctrl}}$ 不能跨 LEO、MEO 和 GEO 通用，需要调度式权重或自适应控制。第四，仿真仍假设连续推力、连续可见性和理想姿态指向，尚未考虑推力器最小脉冲位、相机视场、遮挡和星载计算资源约束。
...
第一，统一 SDC 矩阵具有明确理论基础。由于 SDC 参数化满足 $A(x)x\equiv f(x)$，在 EKF 后验估计点构造的 $A_{\mathrm{SDC}}(\hat{x}_{k|k})$ 可同时作为 EKF 预测矩阵和 SDRE 控制设计矩阵。该方法减少了滤波器和控制器之间的模型失配，使估计与控制围绕同一非线性动力学表示闭环工作。
...
后续研究可从四个方向推进。第一，开展轨道域自适应调参，根据轨道高度、平均角速度、Coriolis 加速度和相对距离调度 $Q_{\mathrm{ctrl}}$ 与 $R_{\mathrm{ctrl}}$，使方法从 MEO 扩展到 LEO 与 GEO。第二，研究仅测角可观测性管理，在 EKF 中引入发散检测、协方差重置、创新门限和主动可观测性机动。第三，比较不同 SDC 参数化，量化 $A_{\mathrm{SDC}}$ 非唯一性对 ARE 解、控制消耗和滤波一致性的影响。第四，开展硬件在环验证，将连续推力控制转换为真实推力器指令，并评估处理器算力、任务时延和传感器视场对闭环性能的影响。
