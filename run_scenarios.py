...
REF_SIGMA_VEL = 1e-3           # 基准速度误差 (km/s) @ REF_DIST  (1 m/s)


# ─── 场景定义（ECI 绝对状态，km / km/s）────────────────────────────────────────
...
def eci_to_lvlh_scenario(cfg: dict) -> tuple[np.ndarray, np.ndarray, float]:
    """将场景 ECI 初始状态转换为 LVLH 坐标，返回 (X_p0_lvlh, X_e0_lvlh, nu0)。"""
    orb = cfg["chief_orbit"]
    a  = orb["a"]
    e  = orb["e"]
    i  = orb["i"]     * DEG2RAD
    ...
    # 从目标 ECI 位置反推真近点角 nu0（i=0, Omega=0, omega=0 时 nu = arctan2(y,x)）
...
def create_ekf(noisy: bool, x0: np.ndarray, initial_dist: float) -> RelativeStateEKF:
    ...
    位置误差标准差 = REF_SIGMA_POS * (initial_dist / REF_DIST) * SIGMA_ANG / SIGMA_ANG
                   = REF_SIGMA_POS * initial_dist / REF_DIST
    速度误差标准差 = REF_SIGMA_VEL * initial_dist / REF_DIST
    """
    if noisy:
        scale = initial_dist / REF_DIST
        sigma_pos = REF_SIGMA_POS * scale   # km
        sigma_vel = REF_SIGMA_VEL * scale   # km/s
        P0 = np.diag([sigma_pos**2] * 3 + [sigma_vel**2] * 3)
        R_meas = np.diag([1e10, SIGMA_ANG**2, SIGMA_ANG**2])
        Q_proc = np.diag([5e-4, 5e-4, 5e-4, 5e-8, 5e-8, 5e-8])
    else:
        P0     = np.diag([1.0, 1.0, 1.0, 1e-4, 1e-4, 1e-4])
        R_meas = np.diag([1e10, 1e-30, 1e-30])
        Q_proc = np.zeros((6, 6))
...
def run_scenario(key: str, cfg: dict, noisy: bool, out_dir: Path, seed: int = 42):
    ...
    print(f"\n{'='*60}")
    print(f"场景: {cfg['name']}  |  初始距离: {initial_dist:.1f} km  |  {'有噪声' if noisy else '准确测量'}")
